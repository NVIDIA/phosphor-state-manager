...
phosphor::state::manager::utils::PropertyValue StateMachineHandler::handleTimeoutRetries(
    ...
    const std::string& property)
{
    ...
    while (retryCount < maxRetries)
    {
        try
        {
            ...
            log<level::INFO>(
                (boost::format("Successfully retrieved property '%s' from object '%s', interface '%s' on retry attempt %d.") %
            ...
            if (std::string(e.name()) == "org.freedesktop.DBus.Error.Timeout")
            {
                log<level::WARNING>(
                    (boost::format(
                     ...
                     property % objectPath % interface % (retryCount + 1) % maxRetries)
                        .str()
                        .c_str());
            }
            else
        ...
        for (const Condition& condition : stateValueTransition.conditions)
        {
                 ...
                 servicesToBeMonitored[condition.intf])
            {
                ...
                try
                {
                    ...
                    {
                        tmp = handleTimeoutRetries(bus, objectPath, condition.intf, condition.property);
                    }
                }
